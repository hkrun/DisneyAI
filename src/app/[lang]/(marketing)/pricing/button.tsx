...
export function PaymentButton({ btnlabel, lang, mode, product, currency, paymentTips, authErrorTitle, authErrorDesc, authTexts, paymentTexts, isFreeTrial = false, i18nPricing, isPopular = false, planType = 'basic', billingPeriod = 'monthly' }: Props) {
    ...
    const getButtonLabel = () => {
        ...
        // 个人版订阅版本（basic, premium, professional）直接显示立即订阅
        if (planType === 'basic' || planType === 'premium' || planType === 'professional') {
            return i18nPricing?.subscription?.subscribeNow || "立即订阅";
        }
    ...
    // 🔍 预检查：用户登录后立即检查是否已使用试用（仅非个人版订阅版本需要检查）
    useEffect(() => {
        async function checkSubscription() {
            if (session?.user && !planType?.startsWith('oneTime') && !['basic', 'premium', 'professional'].includes(planType || '')) {
                try {
        ...
        checkSubscription();
    }, [session?.user?.id, planType]); // 仅非个人版订阅版本检查试用状态
        ...
        <>
            <Dialog open={isOpen} onOpenChange={setIsOpen}>
                <DialogContent className={`sm:max-w-[1024px] border-primary-gold/30 ${styles.lightDialog}`}>
                    ...
                    isFreeTrial={planType === 'freeTrial' ? true : false}/>}
                </DialogContent>
            </Dialog>
                ...
                <AlertDialogContent>
                    ...
                    <AlertDialogFooter>
                        <AlertDialogAction
                            ...
                            className="bg-disney-red text-white hover:bg-disney-red/90"
                        >
